...
export default function ShantiMantra({ slug, compact = false }: { slug: string; compact?: boolean }) {
  ...
  return <section className={`mantra-panel ${compact ? "compact" : ""}`}>
    <div className="mantra-heading"><div><p className="eyebrow">Śānti mantra · peace verse · {mantra.veda}</p><h2>{mantra.title}</h2></div><div className="script-toggle" aria-label="Mantra script"><button className={script === "devanagari" ? "active" : ""} onClick={() => setScript("devanagari")}>Sanskrit script</button><button className={script === "transliteration" ? "active" : ""} onClick={() => setScript("transliteration")}>Roman letters (IAST)</button></div></div>
    <p>A śānti mantra is a verse recited to ask for peace before study. Read it in Devanagari, the Sanskrit script shown here, or in IAST, a way of writing Sanskrit sounds in Roman letters with marks such as ā and ś.</p>
    ...
    {!compact && <><div className="mantra-meaning"><span>In essence</span><p>{mantra.meaning}</p></div>{mantra.note && <p className="tradition-note">Reading note: {mantra.note}</p>}</>}
